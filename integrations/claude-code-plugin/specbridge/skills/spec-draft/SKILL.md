...
1. Start discovery in this same conversation with the `spec_intake_start`
   MCP tool. Pass the confirmed draft's full Markdown as `specification`
   (verbatim and complete), not the file path. The user should not have to
   switch to a terminal between "looks good" and the product questions.
