---
name: status
description: "Show SpecBridge spec status — list all specs, or show one spec's workflow state, stage approvals, stale-approval detection, and task progress, with the next valid workflow step. Use when the user asks where a spec stands or what to do next. Read-only. Natural-language triggers include \"现在跑到哪里了？\", \"現在跑到哪裡了？\", and requests for job progress."
---
